{ config, lib, ... }:
let
  cfg = config.probe-rs.udev-rules;
in
{
  options.probe-rs.udev-rules = {
    enable = lib.mkEnableOption "Add the udev rules for probe-rs";
  };
  config = lib.mkIf cfg.enable {
    services.udev.extraRules = ''
      # From the https://probe.rs/files/69-probe-rs.rules 
